...
ERRORS = {
    ...
    'unexp_end_indentle': u'Unexpected end of indentle while parsing Lua string.',
...
class SLPP(object):
    ...
    def encode(self, obj):
        ...
        if self.variable:
            return '%s =%s -- end of %s\n' % (self.variable, self.__encode(obj), self.variable)
        return self.__encode(obj)

    def __encode(self, obj, key_name=None):
        s = ''
        indent = self.indent
        ...
        if isinstance(obj, str):
            ...
            self.depth += 1
            dp = indent * self.depth
            if self.space_after_equals:
                s += " "
            s += "%s%s{%s" % (newline, indent * (self.depth-1), newline)
            if isinstance(obj, dict):
                key_list = ['[%s]' if isinstance(k, Number) else '["%s"]' for k in obj.keys()]
                contents = [dp + (key + ' =%s') % (k, self.__encode(v, k)) for (k, v), key in zip(obj.items(), key_list)]
                if contents:
                    s += ('%s' % newline).join(contents) + newline
            else:
                s += (',%s' % newline).join(
            ...
            comment = ''
            if key_name:
                comment = ' -- end of %s' % ('[%s]' if isinstance(key_name, Number) else '["%s"]') % key_name

            s += "%s}%s%s" % (indent * self.depth, "," if self.depth else '', comment)
            ...
            while self.ch:
                ...
                if self.ch == '{':
                    ...
                    if ch in ('=', ','):
                        ...
                        idx += 1
                        k = None
        raise ParseError(ERRORS['unexp_end_indentle'])  # Bad exit here
    ...
    def word(self):
        ...
        # At this point we have a word break, now glob up any whitespace, newlines
        while self.ch is not None and (self.space.match(self.ch) or self.ch == self.newline):
            self.next_chr()

        # If we find ourselves facing an "=", save our variable, and start again
        if self.ch == "=":

            self.variable = s

            # Move past the character and start processing again
            self.next_chr()

            # Silly DCS encoder
            if self.ch == " ":
                self.space_after_equals = True
